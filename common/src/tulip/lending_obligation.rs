use super::lending_error::LendingError;
use super::lending_last_update::LastUpdate;
use crate::math::{
...
impl Obligation {
    ...
    pub fn pseudo_loan_to_value(&self) -> Result<Decimal> {
        ...
        Ok(self.borrowed_value.try_div(self.deposits_market_value)?)
    }

    /// Calculate the current ratio of borrowed value to deposited value
    pub fn loan_to_value(&self) -> Result<Decimal> {
        Ok(self.borrowed_value.try_div(self.deposits_market_value)?)
    }

    /// Repay liquidity and remove it from borrows if zeroed out
    pub fn repay(&mut self, settle_amount: Decimal, liquidity_index: usize) -> Result<()> {
    ...
    pub fn max_liquidation_amount(&self, _liquidity: &ObligationLiquidity) -> Result<Decimal> {
...
impl IsInitialized for Obligation {
    fn is_initialized(&self) -> bool {
        self.version != super::LENDING_UNINITIALIZED_VERSION
    }
}
...
impl ObligationLiquidity {
    ...
    pub fn repay(&mut self, settle_amount: Decimal) -> Result<()> {
        self.borrowed_amount_wads = self.borrowed_amount_wads.try_sub(settle_amount)?;
        Ok(())
    }

    /// Increase borrowed liquidity
    pub fn borrow(&mut self, borrow_amount: Decimal) -> Result<()> {
        self.borrowed_amount_wads = self.borrowed_amount_wads.try_add(borrow_amount)?;
        Ok(())
    }

    /// Accrue interest
    pub fn accrue_interest(&mut self, cumulative_borrow_rate_wads: Decimal) -> Result<()> {
...
impl Pack for Obligation {
    ...
    fn pack_into_slice(&self, dst: &mut [u8]) {
        ...
        *last_update_slot = self.last_update.slot.to_le_bytes();
        ...
        super::pack_decimal(self.borrowed_value, borrowed_value);
        ...
        *pc_deposits = self.pc_deposits.to_le_bytes();
        super::pack_decimal(self.deposits_market_value, deposits_market_value);
        ...
        *borrows_len = u8::try_from(self.borrows.len()).unwrap().to_le_bytes();
        ...
        for collateral in &self.deposits {
            ...
            *deposited_amount = collateral.deposited_amount.to_le_bytes();
            super::pack_decimal(collateral.market_value, market_value);
        ...
        for liquidity in &self.borrows {
            ...
            super::pack_decimal(liquidity.borrowed_amount_wads, borrowed_amount_wads);
            super::pack_decimal(liquidity.market_value, market_value);
    ...
    fn unpack_from_slice(src: &[u8]) -> std::result::Result<Self, ProgramError> {
        ...
        if version > super::LENDING_PROGRAM_VERSION {
        ...
        for _ in 0..deposits_len {
            ...
            deposits.push(ObligationCollateral {
                ...
                market_value: super::unpack_decimal(market_value),
            });
            offset += OBLIGATION_COLLATERAL_LEN;
        }
        for _ in 0..borrows_len {
            ...
            borrows.push(ObligationLiquidity {
                ...
                cumulative_borrow_rate_wads: super::unpack_decimal(cumulative_borrow_rate_wads),
                borrowed_amount_wads: super::unpack_decimal(borrowed_amount_wads),
                market_value: super::unpack_decimal(market_value),
            });
            offset += OBLIGATION_LIQUIDITY_LEN;
        }
        ...
        Ok(Self {
            ...
            deposits,
            borrows,
            borrowed_value: super::unpack_decimal(borrowed_value),
            ...
            deposits_market_value: super::unpack_decimal(deposits_market_value),
...
mod test {
    ...
    fn obligation_accrue_interest_failure() {
        ...
        .expect_err("Math operation overflow");
