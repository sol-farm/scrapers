use super::lending_obligation::{Obligation, ObligationCollateral, ObligationLiquidity};
use super::{
    lending_error::LendingError, lending_last_update::LastUpdate, LENDING_PROGRAM_VERSION,
    LENDING_UNINITIALIZED_VERSION,
};
use super::{LENDING_INITIAL_COLLATERAL_RATE, LENDING_SLOTS_PER_YEAR};
use crate::math::{
...
use crate::port::port_variable_rate_lending_instructions::{
    math::Rate as PortRate, state::CollateralExchangeRate as PortCollateralExchangeRate,
};
...
use solend_token_lending::{
    math::Rate as SolendRate, state::CollateralExchangeRate as SolendCollateralExchangeRate,
};
use spl_token_lending::{
    math::Rate as UnmodifiedRate, state::CollateralExchangeRate as UnmodifiedCollateralExchangeRate,
};
...
impl Reserve {
    ...
    pub fn init(&mut self, params: InitReserveParams) {
        self.version = super::LENDING_PROGRAM_VERSION;
    ...
    pub fn withdraw_platform_fees(&mut self) -> Result<u64> {
    ...
    /// Borrow liquidity
    pub fn margin_calculate_borrow(&self, amount_to_borrow: u64) -> Result<CalculateBorrowResult> {
        // @TODO: add lookup table https://git.io/JOCYq
        let receive_amount = amount_to_borrow;
        let borrow_amount = Decimal::from(receive_amount);
        let (borrow_fee, host_fee) = self
...
impl ReserveLiquidity {
    ...
    pub fn total_supply(&self) -> Result<Decimal> {
        Ok(Decimal::from(self.available_amount)
    ...
    pub fn repay(&mut self, repay_amount: u64, settle_amount: Decimal) -> Result<()> {
        self.available_amount = self
            .available_amount
    ...
    pub fn utilization_rate(&self) -> Result<Rate> {
        ...
        let rate = Rate::try_from(self.borrowed_amount_wads.try_div(total_supply)?)?;
        Ok(rate)
    }

    /// Compound current borrow rate over elapsed slots
    fn compound_interest(&mut self, current_borrow_rate: Rate, slots_elapsed: u64) -> Result<()> {
        let slot_interest_rate = current_borrow_rate.try_div(LENDING_SLOTS_PER_YEAR)?;
...
impl ReserveCollateral {
    ...
    fn exchange_rate(&self, total_liquidity: Decimal) -> Result<CollateralExchangeRate> {
        let rate = if self.mint_total_supply == 0 || total_liquidity == Decimal::zero() {
            Rate::from_scaled_val(LENDING_INITIAL_COLLATERAL_RATE)
        } else {
...
impl CollateralExchangeRate {
    ...
    pub fn collateral_to_liquidity(&self, collateral_amount: u64) -> Result<u64> {
        Ok(Decimal::from(collateral_amount)
            .try_div(self.0)?
            .try_floor_u64()?)
    }

    /// Convert reserve collateral to liquidity
    pub fn decimal_collateral_to_liquidity(&self, collateral_amount: Decimal) -> Result<Decimal> {
        Ok(collateral_amount.try_div(self.0)?)
    }

    /// Convert reserve liquidity to collateral
    pub fn liquidity_to_collateral(&self, liquidity_amount: u64) -> Result<u64> {
        Ok(self.0.try_mul(liquidity_amount)?.try_floor_u64()?)
    }

    /// Convert reserve liquidity to collateral
    pub fn decimal_liquidity_to_collateral(&self, liquidity_amount: Decimal) -> Result<Decimal> {
...
/// non standard function for usage with port
impl From<UnmodifiedCollateralExchangeRate> for CollateralExchangeRate {
    fn from(val: UnmodifiedCollateralExchangeRate) -> Self {
        let val_rate: UnmodifiedRate = val.into();
        let scaled_val = val_rate.to_scaled_val();
        Self(Rate::from_scaled_val_big(scaled_val))
    }
}

/// non standard function for usage with solend
impl From<SolendCollateralExchangeRate> for CollateralExchangeRate {
    fn from(val: SolendCollateralExchangeRate) -> Self {
        let val_rate: SolendRate = val.into();
        let scaled_val = val_rate.to_scaled_val();
        Self(Rate::from_scaled_val_big(scaled_val))
    }
}

impl From<PortCollateralExchangeRate> for CollateralExchangeRate {
    fn from(val: PortCollateralExchangeRate) -> Self {
        let val_rate: PortRate = val.into();
        let scaled_val = val_rate.to_scaled_val();
        Self(Rate::from_scaled_val_big(scaled_val))
    }
}
...
impl ReserveFees {
    ...
    pub fn calculate_flash_loan_fees(&self, flash_loan_amount: Decimal) -> Result<(u64, u64)> {
        self.calculate_fees(
            flash_loan_amount,
...
impl IsInitialized for Reserve {
    fn is_initialized(&self) -> bool {
        self.version != LENDING_UNINITIALIZED_VERSION
    }
}
...
impl Pack for Reserve {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        ...
        *last_update_slot = self.last_update.slot.to_le_bytes();
        super::pack_bool(self.last_update.stale, last_update_stale);
        ...
        *liquidity_available_amount = self.liquidity.available_amount.to_le_bytes();
        ...
        super::pack_decimal(self.liquidity.market_price, liquidity_market_price);
        super::pack_decimal(
    ...
    fn unpack_from_slice(input: &[u8]) -> std::result::Result<Self, ProgramError> {
        ...
        if version > LENDING_PROGRAM_VERSION {
        ...
        Ok(Self {
            ...
            last_update: LastUpdate {
                ...
                stale: super::unpack_bool(last_update_stale)?,
            },
            ...
            liquidity: ReserveLiquidity {
                ...
                borrowed_amount_wads: super::unpack_decimal(liquidity_borrowed_amount_wads),
                cumulative_borrow_rate_wads: super::unpack_decimal(
                    liquidity_cumulative_borrow_rate_wads,
                ),
                market_price: super::unpack_decimal(liquidity_market_price),
                platform_amount_wads: super::unpack_decimal(liquidity_platform_amount_wads),
